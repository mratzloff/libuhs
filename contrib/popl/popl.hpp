...
public:
	...
	/// @param long_name the option's long name. Can be empty.
	/// @param description the Option's description that will be shown in the help message
	Value(const std::string& short_name, const std::string& long_name,
...
public:
	Switch(const std::string& short_name, const std::string& long_name,
...
/**
 * OptionParser manages all Options
 * Add Options (Option_Type = Value<T>, Implicit<T> or Switch) with
 * "add<Option_Type>(option params)" Call "parse(argc, argv)" to trigger parsing of the
 * options and to fill "non_option_args" and "unknown_options"
 */
class OptionParser {
public:
	...
	/// e.g. "-i 5 hello" => hello
	/// e.g. "-i 5 -- from here non option args" => "from", "here", "non", "option",
	/// "args"
...
public:
	...
	invalid_option(const Option* option, invalid_option::Error error,
	    OptionName what_name, std::string value, const std::string& text)
	    : std::invalid_argument(text.c_str())
	    , option_(option)
	    , error_(error)
	    , what_name_(what_name)
	    , value_(std::move(value)) {}

	invalid_option(
	    const Option* option, invalid_option::Error error, const std::string& text)
...
public:
	...
	virtual std::string print(
	    const Attribute& max_attribute = Attribute::optional) const = 0;
...
public:
	...
	std::string print(
	    const Attribute& max_attribute = Attribute::optional) const override;
...
public:
	BashCompletionOptionPrinter(
	    const OptionParser* option_parser, std::string program_name);
...
inline Option::Option(
    const std::string& short_name, const std::string& long_name, std::string description)
	...
	if (short_name.size() > 1)
		throw std::invalid_argument(
		    "length of short name must be <= 1: '" + short_name + "'");
...
template<class T>
inline Value<T>::Value(const std::string& short_name, const std::string& long_name,
    const std::string& description)
...
inline void Value<std::string>::parse(OptionName what_name, const char* value) {
	if (strlen(value) == 0)
		throw invalid_option(this,
		    invalid_option::Error::missing_argument,
		    what_name,
		    value,
		    "missing argument for " + name(what_name, true));
...
inline void Value<T>::parse(OptionName what_name, const char* value) {
	...
	if (is.fail())
		throw invalid_option(this,
		    invalid_option::Error::invalid_argument,
		    what_name,
		    value,
	...
	if (valuesRead > 1)
		throw invalid_option(this,
		    invalid_option::Error::too_many_arguments,
		    what_name,
		    value,
...
template<class T>
inline Implicit<T>::Implicit(const std::string& short_name, const std::string& long_name,
...
inline Switch::Switch(const std::string& short_name, const std::string& long_name,
...
inline std::shared_ptr<T> OptionParser::add(Ts&&... params) {
	...
	for (const auto& o : options_) {
		if ((option->short_name() != 0) && (option->short_name() == o->short_name()))
			throw std::invalid_argument("duplicate short option name '-"
...
inline std::shared_ptr<T> OptionParser::get_option(char short_name) const {
	...
	if (!result)
		throw std::invalid_argument(
		    "cannot cast option to T: " + std::string(1, short_name));
	return result;
}
...
inline void OptionParser::parse(const std::string& ini_filename) {
	...
	auto split = [trim_copy](
	                 const std::string& s) -> std::pair<std::string, std::string> {
		size_t pos = s.find('=');
		if (pos == std::string::npos)
			return {"", ""};
		return {
		    trim_copy(s.substr(0, pos)), trim_copy(s.substr(pos + 1, std::string::npos))};
	};
	...
	while (std::getline(file, line)) {
		...
		std::string key =
		    section.empty() ? key_value.first : section + "." + key_value.first;
...
inline void OptionParser::parse(int argc, const char* const argv[]) {
	...
	for (auto& opt : options_) {
		if ((opt->attribute() == Attribute::required) && !opt->is_set()) {
			std::string option = opt->long_name().empty()
			                         ? std::string(1, opt->short_name())
			                         : opt->long_name();
			throw invalid_option(opt.get(),
			    invalid_option::Error::missing_option,
...
inline std::string BashCompletionOptionPrinter::print(
    const Attribute& /*max_attribute*/) const {
	if (option_parser_ == nullptr)
		return "";
