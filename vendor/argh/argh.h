...
public:
	...
	void parse(
	    int argc, const char* const argv[], int mode = PREFER_FLAG_FOR_UNREG_OPTION);
	...
	// multiple flag (boolean) accessors: return true if at least one of the flag
	// appeared, otherwise false.
	...
	// returns a std::istream that can be used to convert a positional arg to a typed
	// value.
	string_stream operator()(size_t ind) const;

	// same as above, but with a default value in case the arg is missing (index out of
	// range).
	template<typename T>
	string_stream operator()(size_t ind, T&& def_val) const;

	// parameter accessors, give a name get an std::istream that can be used to convert to
	// a typed value. call .str() on result to get as string
	...
	// std::istream that can be used to convert to a typed value. call .str() on result to
	// get as string returns the first value in the list to be found.
	string_stream operator()(std::initializer_list<char const* const> init_list) const;

	// same as above, but with a default value in case the param was missing.
	// Non-string def_val types must have an operator<<() (output stream operator)
	// If T only has an input stream operator, pass the string version of the type as in
	// "3" instead of 3.
	template<typename T>
...
inline void parser::parse(
	...
	std::transform(
	    argv, argv + argc, args_.begin(), [](const char* const arg) { return arg; });
	...
	for (auto i = 0u; i < args_.size(); ++i) {
		...
		// any potential option will get as its value the next arg, unless that arg is an
		// option too in that case it will be determined a flag.
		if (i == args_.size() - 1 || is_option(args_[i + 1])) {
			flags_.emplace(name);
			continue;
		}

		// if 'name' is a pre-registered option, then the next arg cannot be a free
		// parameter to it is skipped otherwise we have 2 modes:
		// PREFER_FLAG_FOR_UNREG_OPTION: a non-registered 'name' is determined a flag.
		//                               The following value (the next arg) will be a free
		//                               parameter.
		//
		// PREFER_PARAM_FOR_UNREG_OPTION: a non-registered 'name' is determined a
		// parameter, the next arg
...
inline bool parser::is_number(std::string const& arg) const {
	// inefficient but simple way to determine if a string is a number (which can start
	// with a '-')
...
inline bool parser::operator[](std::initializer_list<char const* const> init_list) const {
	return std::any_of(init_list.begin(), init_list.end(), [&](char const* const name) {
		return got_flag(name);
	});
}
